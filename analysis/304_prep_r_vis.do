...
import delimited using ./output/tables/anx_cox_models.txt, stringcols(3) clear
tempfile tempfile
...
forvalues i=1/10 {
	...
	import delimited using ./output/tables/`outcome'_cox_models.txt, stringcols(3) clear
	tempfile tempfile
	* Generate risk differences between waves for each ethnic group
	* count to reorder
	gen number = _n
	...
	* Other ethnic group 
	...
	merge 1:1 period ethnic_group using `tempfile'
	append using "./output/tables/all_cox_data"
	save "./output/tables/all_cox_data", replace	
...
export delimited using "$savedir\all_cox_data_inc_ci.csv", replace
