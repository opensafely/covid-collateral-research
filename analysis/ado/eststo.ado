*! version 1.0.4, Ben Jann, 03may2007
...
// - loop over bygroups
	forv i = 1/`Nby' {
		...
		capture noisily {
			_eststo_store `0'`ibytitle' : `estcmd'
		}
		if _rc {
			if "`_byrc0'"=="" error _rc
		}
...
program define _eststo_drop
	...
	foreach item of local droplist {
		...
		if `:list dropname in names' {
			estimates drop `dropname'
			local names: list names - dropname
			di as txt "(" as res "`dropname'" as txt " dropped)"
		}
		else di as txt "(" as res "`dropname'" as txt " not found)"
	}
	global eststo `names'
end
...
// refresh2 provided => check availability
				...
				if `:list posof "est`refresh2'" in names' {
					local name est`refresh2'
				}
				else {
					di as txt "(" as res "est`refresh2'" as txt " not found)"
					local refresh
				}
			}
		}
		if "`refresh'"=="" local ++counter
// set default name
		if "`name'"=="" local name est`counter'
	}
