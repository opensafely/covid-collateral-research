...
// - tex
	local tex_open0           `""% `cdate' `ctime'" \documentclass{article} \`texpkgs' \begin{document} """'
...
// - booktabs
	local booktabs_open0      `""% `cdate' `ctime'" \documentclass{article} \`texpkgs' \usepackage{booktabs} \begin{document} """'
...
// format modes
	...
	if "`compress'"!="" & !inlist("`mode0'", "tab", "csv") {
		if "`modelwidth'"=="" local modelwidth 9
		if "`varwidth'"=="" {
			local varwidth = cond("`label'"!="", "16", "10")
		}
	}
	foreach opt in varwidth modelwidth {
		if `"``opt''"'=="" {
			local `opt' `"``mode'_`opt''"'
		}
	}
	if "`plain'"=="" {
...
// compose note
			if "`notes'"!="" {
				...
				if `"`auxname'"'!="" {
					...
					local note `"`macval(note)'``mode'_tstatlab'"'
				}
				else if `"`aux'"'=="se" {
					local note `"`macval(note)'Standard errors"'
				}
				else if `"`aux'"'=="p" {
					local note `"`macval(note)'``mode'_pvallab'"'
				}
				else if `"`aux'"'=="ci" {
					local note `"`macval(note)'``mode'_cilab'"'
				}
				if "`parentheses'"=="" {
...
// table header
	...
	if `"`macval(collabels)'"'=="" & `nocellsopt' & "`plain'"=="" {
...
// execute estout
	...
	CleanEstoutCmd `anything' `using' ,  ///
	 ...
	 `macval(begin)' `macval(delimiter)' `macval(end)' `smcltags'                 ///
	 ...
	 `macval(substitute)' `append' `notype'`type' level(`level') `macval(options)'
	if "`noisily'"!="" {
...
///	 NOWRAP wrap ///
	...
	foreach opt in ///
	 ...
	 nonumbers numbers collabels eqlabels mgroups append notype type ///
	 options {
...
prog ParseEqLabels
	syntax [anything] [ , Begin(passthru) ///
	 NOReplace Replace NOFirst First * ]
	c_local eqlabelsok = `"`begin'`noreplace'`replace'`nofirst'`first'"'==""
	if `"`options'"'=="" c_local eqlabelscomma ", "
	else c_local eqlabelscomma " "
end
