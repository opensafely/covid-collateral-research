...
cap mkdir ./output/measures/dm/collapsed
cap mkdir ./output/graphs/dm

* Generates graphs for clinical monitoring measures
local a "hba1c systolic_bp t1_primary t1_any t2_primary t2_any keto_primary keto_any"
local b "population population has_t1_diabetes has_t1_diabetes has_t2_diabetes has_t2_diabetes population population"
forvalues i=1/8 {
    local c: word `i' of `a'
    local d: word `i' of `b'
    * Ethnicity
    import delimited using ./output/measures/dm/measure_dm_t1_`v'_ethnicity_rate.csv, numericcols(4) clear
    ...
    * Collapse at 3 monthly intervals
    ...
    drop quarter
    * Outputing file 
    export delimited using ./output/measures/dm/collapsed/collapse_measure_dm_t1_`v'_ethnicity_rate.csv, replace
    ...
    * Generate line graph
    ...
    graph export ./output/graphs/dm/line_dm_ethnic_t1_`v'.eps, as(eps) replace
    * IMD - type 1 DM
    clear 
