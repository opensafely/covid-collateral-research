...
forvalues i=1/2 {
	...
	*Autoregression plots by ethnicity
    ac rate if `d'==1, normal name(ac_`d'_1_`x')
	ac rate if `d'==2, normal name(ac_`d'_2_`x')
    ac rate if `d'==3, normal name(ac_`d'_3_`x')
    ac rate if `d'==4, normal name(ac_`d'_4_`x')
    ac rate if `d'==5, normal name(ac_`d'_5_`x')
	*Partial autoregression plots by ethnicity
	pac rate if `d'==1, normal name(pac_`d'_1_`x')
	pac rate if `d'==2, normal name(pac_`d'_2_`x')
    pac rate if `d'==3, normal name(pac_`d'_3_`x')
    pac rate if `d'==4, normal name(pac_`d'_4_`x')
    pac rate if `d'==5, normal name(pac_`d'_5_`x')
	*Combine Graphs
	graph combine kd_`d'*, altshrink
	graph export .output/graphs/checks_kd_`d'.eps, as(eps) replace
	graph combine ac_`d'*, altshrink
