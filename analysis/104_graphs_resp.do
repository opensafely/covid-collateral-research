...
forvalues i=1/2 {
    ...
    * Generate line graph
    ...
    angle(0)) yscale(titlegap(*10)) ytick(0(1000)8000) xmtick(##6) legend(row(1) size(small) title("Ethnic categories", size(small)))
    ...
    * Generate line graph
    graph twoway line rate1 rate2 rate3 rate4 rate5 date, tlabel(01Jan2018(180)01Jan2022, angle(45) ///
    format(%dM-CY) labsize(small)) ytitle("Rate per 100,000") xtitle("Date") ylabel(, labsize(small) ///
    angle(0)) yscale(titlegap(*10)) ytick(0(1000)8000) xmtick(##6) legend(row(1) size(small) title("IMD categories", size(small)))
